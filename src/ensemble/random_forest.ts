...
function bootstrapSample<T, U>(
	X: T[],
	y: U[],
): { X_sample: T[]; y_sample: U[] } {
	const n = X.length;
	const X_sample: T[] = [];
	const y_sample: U[] = [];
	for (let i = 0; i < n; i++) {
...
export interface RandomForestParameters<
...
> {
	...
	protected abstract createTree(
		params: DecisionTreeParameters<Y_IN>,
	): TREE_TYPE;
	...
	/**
	 * Returns the Random Forest structure as a JSON string.
	 *
	 * @returns A JSON string representation of the Random Forest.
	 */
	...
	public fromJSON(json: string): void {
		...
		this.nEstimators = data.nEstimators;
		this.bootstrap = data.bootstrap;
		this.treeParams = data.treeParams;
		this.trees = data.trees.map((tree: string) => {
			const treeInstance = this.createTree(this.treeParams);
			treeInstance.fromJSON(tree);
			return treeInstance;
		});
	}
}
		...
		for (let i = 0; i < X.length; i++) {
			...
			for (const [label, count] of Object.entries(votes)) {
				if (count > maxCount) {
					...
					if (!Number.isNaN(Number(label))) majorityVote = Number(label);
				}
			}
...
> {
	...
	/**
	 * Parses a JSON string to restore the Random Forest structure.
	 *
	 * @param json - A JSON string representation of the Random Forest.
	 **/
	public fromJSON(json: string): void {
		const data = JSON.parse(json);
		this.nEstimators = data.nEstimators;
